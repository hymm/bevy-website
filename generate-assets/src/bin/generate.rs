...
fn main() -> anyhow::Result<()> {
    ...
    write_search_index(&asset_root_section, &content_dir);
...
#[derive(Serialize)]
struct TinySearchIndex {
    entries: Vec<TinySearchEntry>,
}

#[derive(Serialize)]
struct TinySearchEntry {
    title: String,
    url: String,
    body: String,
}

// write search index for use with tinysearch
fn write_search_index(section: &Section, content_dir: &String) {
    let mut index = TinySearchIndex {
        entries: Vec::new(),
    };

    add_assets_from_section(section, &mut index);

    let json = serde_json::to_string(&index.entries).expect("could not serialize index");
    let index_path = Path::new(content_dir).join("assets/search-index.json");
    fs::write(&index_path, json.as_bytes())
        .unwrap_or_else(|_| panic!("Failed to create search-index.json at {:?}", index_path));
}

fn add_assets_from_section(section: &Section, index: &mut TinySearchIndex) {
    for node in &section.content {
        match node {
            AssetNode::Asset(asset) => index.entries.push(TinySearchEntry {
                title: asset.name.clone(),
                url: slugify(&asset.name), // this might be a problem as slugs are generated with a different library from zola
                body: asset.description.clone(),
            }),
            AssetNode::Section(section) => add_assets_from_section(section, index),
        }
    }
}
